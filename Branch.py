...
class Branch(example_pb2_grpc.RPCServicer):
    ...
    def Deposit(self, request, context):
        ...
        self.Propogate_Deposit(self.balance)
    ...
    def Withdraw(self, request, context):
        ...
        self.Propogate_Withdraw(self.balance)
        return example_pb2.MsgDeliveryReply(id=request.id, money=self.balance, result="success", interface=request.interface)
    ...
    def Propogate_Withdraw(self, balance):
        ...
        for port in range(50051, int(lastBranchPortPlusOne)):
            ...
            rsp = st.MsgDelivery(example_pb2.MsgDeliveryRequest(id=1, money=balance, dest=1, interface="propagate-withdraw"))

    # Propagate the Deposit process
    # Takes in the balance and propagates the balance to all the branches from the starting port number to the last branch port
    def Propogate_Deposit(self, balance):
        print(printableBranch, "- PROPAGATE DEPOSIT")
        for port in range(50051, int(lastBranchPortPlusOne)):
            ch=grpc.insecure_channel('localhost:'+ str(port))
            st=example_pb2_grpc.RPCStub(ch)
            rsp = st.MsgDelivery(example_pb2.MsgDeliveryRequest(id=1, money=balance, dest=1, interface="propagate-deposit"))
...
if __name__ == '__main__':
    ...
    print(printableBranch, " - Main: ", "Branch process started at port: ", str(thisport), "with ID: ", str(branchid), ", balance: ", str(branchbalance), " and LastBranchPort", str(lastBranchPortPlusOne))

    # Create the Branch server process with the ID and the balance
    example_pb2_grpc.add_RPCServicer_to_server(Branch(str(branchid),str(branchbalance),1), server)
